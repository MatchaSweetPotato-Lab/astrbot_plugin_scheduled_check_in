...
class BaseCheckInAdapter(ABC):
    ...
    def __init__(self, site_config: dict[str, Any], session: aiohttp.ClientSession) -> None:
...
class NewApiAdapter(BaseCheckInAdapter):
        ...
        try:
            async with self.session.get(url, headers=headers, proxy=self.proxy, timeout=10) as resp:
                ...
                if "<html" in text.lower() or "acw_sc" in text.lower() or "denied by http_custom" in text.lower():
                    ...
                    return 0.0, True, "被站点 WAF 防火墙拦截 (Aliyun WAF JS Challenge)"
    ...
    async def check_in(self) -> CheckInResult:
        ...
        for endpoint in endpoints:
            try:
                async with self.session.post(endpoint, headers=headers, proxy=self.proxy, timeout=10) as resp:
                    if resp.status in (401, 403):
                        expired = True
                    ...
                    if resp.status == 405:
                        ...
                        async with self.session.get(endpoint, headers=headers, proxy=self.proxy, timeout=10) as get_resp:
                            if get_resp.status == 200:
                                get_text = await get_resp.text()
                                if "<html" not in get_text.lower():
                                    import json
    ...
    async def test_connection(self) -> CheckInResult:
        ...
        if expired:
            ...
            try:
                async with self.session.get(models_url, headers=headers, proxy=self.proxy, timeout=8) as m_resp:
...
class GenericRestAdapter(BaseCheckInAdapter):
    ...
    async def _execute_request(self) -> CheckInResult:
        ...
        try:
            req_func = self.session.post if method == "POST" else self.session.get
            async with req_func(url, headers=headers, proxy=self.proxy, timeout=10) as resp:
...
def create_adapter(site_config: dict[str, Any], session: aiohttp.ClientSession) -> BaseCheckInAdapter:
    ...
    if site_type in ("new-api", "one-api"):
        return NewApiAdapter(site_config, session)
    return GenericRestAdapter(site_config, session)
