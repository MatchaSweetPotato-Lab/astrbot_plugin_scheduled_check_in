...
- **📜 日志明细与抽屉面板**
  ...
  - 支持按日期筛选、查看单次请求链路与响应摘要，并支持一键清空日志。
  - 全局设置支持按**保留天数**和**保留条数**独立清理日志；任一项设为 `0` 表示该条件不限制，两项都设置时同时生效。

- **📅 签到日历与余额变化**
  - 点击站点的今日签到状态即可查看月度签到日历、当前余额和余额变化记录。
...
- `data.db`：SQLite 本地数据库（采用 WAL 高并发模式），统一持久化中转站配置（`sites` 表）、全局设置（`settings` 表）与历史打卡/测试日志（`history_logs` 表，支持日期筛选、游标分页，以及按天/按条数清理）。
